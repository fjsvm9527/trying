...
def main():
    ...
    if new_train_df is not None and new_valid_df is not None:
        ...
        train_corr = new_train_df[[f'vae_latent_{i}' for i in range(cfg['model']['latent_dim'])]].corrwith(new_train_df[label_col])
        valid_corr = new_valid_df[[f'vae_latent_{i}' for i in range(cfg['model']['latent_dim'])]].corrwith(new_valid_df[label_col])
